...
| Transformers LM GPT-J  | 1.5 GB RAM, 24.2 GB GPU | generative service based on Transformers generative model, the model is set in docker compose argument `PRETRAINED_MODEL_NAME_OR_PATH` (in particular, in this service, [GPT-J model](https://huggingface.co/EleutherAI/gpt-j-6B) is used.    |
...
| DFF Dream Persona Prompted Skill | 150 MB RAM                | **[New DFF version]** Prompt-based skill that utilizes given generative service to generate responses based on the given prompt                                                                                                                               |
