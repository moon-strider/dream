...
class TorchTransformersPreprocessor(Component):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if expand_path(vocab_file_path).exists():
            vocab_file_path = expand_path(vocab_file_path)
            if vocab_file_path.is_file():
                vocab_file_path = vocab_file_path.parent
...
class TorchTransformersBatchPreprocessor(Component):
    ...
    def __call__(self, texts_a: List[str], texts_b: List[str] = None) -> List[InputFeatures]:
        ...
        encoding = self.tokenizer.batch_encode_plus(
            tokenizer_input, add_special_tokens=True, pad_to_max_length=True, return_attention_mask=True
        )
