...
from deeppavlov.models.kbqa.utils import extract_year, extract_number, order_of_answers_sorting, make_combs, fill_query
from query_generator_base import QueryGeneratorBase
...
class QueryGenerator(QueryGeneratorBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            wiki_parser=self.wiki_parser,
            rel_ranker=self.rel_ranker,
            entities_to_leave=self.entities_to_leave,
            rels_to_leave=self.rels_to_leave,
            return_answers=self.return_answers,
            *args,
            **kwargs,
        )
    ...
    def __call__(
        ...
        try:
            log.info(f"kbqa inputs {question_batch} {entities_from_ner_batch} {types_from_ner_batch}")
            for question, question_sanitized, template_type, entities_from_ner, types_from_ner, q_type_flag in zip(
                question_batch,
                question_san_batch,
                template_type_batch,
                entities_from_ner_batch,
                types_from_ner_batch,
                q_type_flags_batch,
            ):
                if template_type == "-1":
                    template_type = "7"
                candidate_outputs, template_answer = self.find_candidate_answers(
                    question, question_sanitized, template_type, entities_from_ner, types_from_ner, q_type_flag
                )
        ...
        if self.return_answers:
            answers = self.rel_ranker(
                question_batch, candidate_outputs_batch, entities_from_ner_batch, template_answers_batch
            )
    ...
    def query_parser(
        ...
        triplet_info_list = [
            ("forw" if triplet[2].startswith("?") else "backw", search_source, rel_type)
            for search_source, triplet, rel_type in zip(rels_for_search, query_triplets, rel_types)
        ...
        log.debug(
            f"(query_parser)entity_combs: {entity_combs[:3]}, type_combs: {type_combs[:3]},"
        ...
        if isinstance(candidate_outputs_list, list) and candidate_outputs_list:
            ...
            for combs, confidence, candidate_output in zip(all_combs_list, confidences_list, candidate_outputs_list):
                candidate_outputs += [
                    [combs[0]] + [rel for rel, score in combs[2][:-1]] + output + [confidence]
            ...
            if self.return_all_possible_answers:
                ...
                candidate_outputs = []
                for (_, candidate_rel_comb), candidate_output in candidate_outputs_dict.items():
