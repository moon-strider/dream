...
class KBEntityLinker(Component, Serializable):
    ...
    def __call__(
        ...
        log.info(
            f"entity_substr_batch {entity_substr_batch} templates_batch {templates_batch} context_batch {context_batch}"
        )
        entity_ids_batch = []
        confidences_batch = []
        ...
        for entity_substr_list, template_found, context, entity_types_list in zip(
            entity_substr_batch, templates_batch, context_batch, entity_types_batch
        ):
            entity_ids_list = []
            confidences_list = []
            ...
            if self.types_dict:
                if entity_types:
                    ...
                    candidate_entities = [
                        ...
                        if self.types_dict.get(entity[1], set()).intersection(entity_types)
                    ]
    ...
    def sort_found_entities(
        ...
        if self.use_descriptions:
            ...
            id_to_score = {
                ...
                for _, entity_id, tokens_matched, score, _ in srtd_with_ratios[: self.num_entities_for_bert_ranking]
            }
            ...
            entities_with_scores = [
                (entity_id, id_to_score[entity_id][0], id_to_score[entity_id][1], score) for entity_id, score in scores
            ]
            entities_with_scores = sorted(entities_with_scores, key=lambda x: (x[1], x[2], x[3]), reverse=True)
            entities_with_scores = [
                entity
                for entity in entities_with_scores
                if (entity[3] > self.descr_rank_score_thres or entity[2] == 100.0)
            ]
