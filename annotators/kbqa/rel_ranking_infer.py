...
class RelRankerInfer(Component, Serializable):
    ...
    def __call__(
        ...
        for question, candidate_answers, entities, template_answer in zip(
            questions_list, candidate_answers_list, entities_list, template_answers_list
        ):
            answers_with_scores = []
            answer = "Not Found"
            if self.rank:
                ...
                for i in range(n_batches):
                    ...
                    if questions_batch:
                        ...
                        for j, (answer, confidence, rels_labels) in enumerate(
                            zip(answers_batch, confidences_batch, rels_labels_batch)
                        ):
            ...
            if answers_with_scores:
                ...
                if self.return_sentence_answer:
                    try:
                        ...
                        log.info(f"Error in sentence answer {e}")
    ...
    def rank_rels(self, question: str, candidate_rels: List[str]) -> List[Tuple[str, Any]]:
        ...
        if question is not None:
            ...
            if self.softmax:
                ...
                rels_with_scores = [
                    (rel, softmax_score) for (rel, score), softmax_score in zip(rels_with_scores, softmax_scores)
                ]
